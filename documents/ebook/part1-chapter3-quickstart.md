...
```bash
java -Duser.timezone=UTC -jar ojp-server/target/ojp-server-1.0.0-RC3-RC2-RC1-shaded.jar
```
...
```bash
# Start in background
nohup java -Duser.timezone=UTC -jar ojp-server-1.0.0-RC3-RC2-RC1-shaded.jar \
  > ojp-server.log 2>&1 &
...
# Start OJP Server
docker run --network ojp-network --name ojp-server rrobetti/ojp:1.0.0-RC3-RC2-RC1
