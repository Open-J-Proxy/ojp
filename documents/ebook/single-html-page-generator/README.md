...
- `ebook-single-page-content.js` — bundled markdown fallback for direct `file://` opens
...
The page uses two modes:

1. **Fetch-first mode (default)**: reads chapters from `../*.md` at runtime.
2. **Fallback mode**: if fetch is unavailable (for example when opening with `file://`), it uses `ebook-single-page-content.js`.

## Refresh bundled fallback content
